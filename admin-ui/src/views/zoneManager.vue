	...
	<div class="zone-manager pa-4 flex-wrap">
		<div class="buttons__inline pb-4">
			<div class="page__title">
				<router-link :to="{name: 'DNS manager'}">
					zones</router-link>  / {{$route.params.dnsname}}
			</div>
...
export default {
	...
	methods: {
		createZone(){
			...
			.catch(err => {
				if(err.response.status == 501 || err.response.status == 502 || err.response.status == 500){
					const opts = {
		...
		deleteSelectedZones(){
			if(this.selected.length > 0){
				...
				this.selected.forEach(el => {
					const elementIndex = template[el.location][el.type].findIndex(rm => {
					...
					api.dns.setZone(result)
						.then(()=>{
							this.snackbar.message = `host${this.selected.length == 1 ? "" : "s"} deleted successfully.`
