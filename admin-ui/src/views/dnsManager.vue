...
export default {
	...
	methods: {
		...
		deleteSelectedZones(){
			if(this.selected.length > 0){
				const deletePromices = this.selected.map(el => api.dns.delete(el.original));
				...
				.then(res => {
					...
					this.snackbar.message = `dns${deletePromices.length == 1 ? "" : "es"} deleted successfully.`
	...
	mounted(){
		this.$store.commit('reloadBtn/setCallback', {func: this.$store.dispatch, params: ['dns/fetch']})
	}
}
</script>
