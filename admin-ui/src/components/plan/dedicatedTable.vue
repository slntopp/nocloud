  ...
  methods: {
    fetchAddons({ planCode }) {
      ...
      api.post(`/sp/${uuid}/invoke`, {
        method: 'get_baremetal_options', params: { planCode }
      })
        .then(({ meta: { options } }) => {
          const {
            bandwidth = [],
          ...
          const value = this.setPlans({ plans }).map((addon) => {
            const resource = this.template.resources.find((el) => addon.id === el.key);
    ...
    async changePlan(plan) {
      ...
      for await (const el of this.plans) {
        if (el.sell) {
          ...
          const os = requiredConfiguration.find(
            (el) => el.label.includes('os')
          )?.allowedValues ?? [];
  ...
  created() {
    ...
    this.$store.dispatch('servicesProviders/fetch')
      .then(({ pool }) => {
        ...
        return api.post(`/sp/${sp.uuid}/invoke`, { method: 'get_baremetal_plans' });
      })
      .then(({ meta }) => {
      ...
      .finally(() => { this.$emit('changeLoading') });
  },
  mounted() {
