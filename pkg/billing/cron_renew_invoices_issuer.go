...
func (s *BillingServiceServer) createRenewalInvoice(ctx context.Context, log *zap.Logger, _acc *graph.Account, data []*instanceExpData, defCurr *pb.Currency) error {
	...
	for _, d := range data {
		...
		_, summary, err := s.promocodes.GetDiscountPriceByInstance(inst, false, true)
		if err != nil {
...
func filterInvoices(invoices []*graph.Invoice) []*graph.Invoice {
	...
	for _, inv := range invoices {
		if inv.GetStatus() == pb.BillingStatus_RETURNED || inv.GetStatus() == pb.BillingStatus_CANCELED || inv.GetStatus() == pb.BillingStatus_TERMINATED {
			continue
		}
