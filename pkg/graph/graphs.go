...
func ListAccounts[T Accessible](
	...
	for key, val := range filters {
		if key == "search_param" && collectionName == schema.ACCOUNTS_COL {
			insert += fmt.Sprintf(` FILTER LOWER(node.title) LIKE LOWER("%s") || LOWER(node.data.email) LIKE LOWER("%s") || node._key LIKE "%s" || node.data.company LIKE "%s"`, "%"+val.GetStringValue()+"%", "%"+val.GetStringValue()+"%", "%"+val.GetStringValue()+"%", "%"+val.GetStringValue()+"%")
