...
|| CONTAINS(TO_STRING(node.data.ips_history.private), "%s")`,
			...
			query += ` FILTER (acc.account_group in @account_groups) || (acc.accountGroup in @account_groups) || ("" in @account_groups AND (IS_NULL(acc.account_group) OR acc.account_group == "") AND (IS_NULL(acc.accountGroup) OR acc.accountGroup == ""))`
