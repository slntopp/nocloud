...
arango_dump_command="/usr/bin/arangodump \
                     --all-databases \
                     --overwrite \
                     --output-directory $arangodump_dir \
...
docker cp $arango_container_name:$arangodump_dir $host_backup_path              # Copy dump from container to host
docker exec -d $arango_container_name rm -rf $arangodump_dir                    # Remove tmp dir from container
